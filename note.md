...
## 2.1 泛型和函数
回顾一下，在c++中，我们如何编写一个对列表求和的函数？
一般是这样：
```c
int Sum(int data[],size_t num)
{
    int i = 0;
    int sum = 0;
    for(i = 0;i < num;i++)
...
```
我们可以用学过的c++语法来稍加优化对不对，比如加上一个元变量替换数组的类型，还有求和的类型，是不是可以实现函数的泛用？就像这样：
```cpp
template<class SUM>//SUM是一个genetic类型
SUM Sum(const SUM data[],size_t num)
{
    SUM sum = 0;
    for(int i = 0;i < num;i++)
        sum += data[i];//前提是类型SUM存在加法的定义
    return sum;
}
```
(不要忘记加上const，这是一个很好的习惯)
但是不仅仅是这样。目标如下，有两个列表，比如第一个是1 1 2 3 5，第二个是8 13 21，对第一个求和之后，在此基础上，对第二个数组求和。我们知道，第一次求和肯定是在0的基础上相加，我希望第二次求和能在第一次求和的基础上相加。

明确一下，也就是，第一次在0的基础上，第二次在其他数的基础上，也就是说，基础要可变。

于是c++中出现了这样的写法：
```c
template<class SUM>//SUM是一个genetic类型
SUM Sum(const SUM data[],size_t num,SUM sum = 0)
{
    for(int i = 0;i < num;i++)
        sum += data[i];//前提是类型SUM存在加法的定义
    return sum;
...
而这种写法通常在c语言语法识别中被视为错误。

那么这是什么意思呢？既是参数，可以被传入数据，又被赋值？

这代表sum的默认值是0。我们知道sum代表相加的基础。如果我们在第三个参数选择上传值（也就是这个参数可以选择不上传数据，那么它就保持默认值0），那么sum就被赋值成这个值了。自然，相加的基础就成了这个值。

这种参数在c++中被称为“Default parameter -- 默认参数”。

![alt text](image-7.png)
![alt text](image-8.png)

## 2.1 小结
我们发现，c++中存在很多方法可以使得OO的程度变高。比如template<class T>，比如默认参数，还有重载。它们让程序内容更加模块化，比如我在c语言中需要一个整形列表求和，我便针对整形列表写一个求和函数。但是在c++中，通过这些泛型，我可以写一个任何类型都可能可以使用的列表求和，然后我只需要根据所需上传不同的参数，通过重载自动帮我选择合适的函数。这就导致c中的函数是与我的需求高度绑定，而c++的函数则像是一个万能工具。
...
函数模板被广泛使用后的引进的形式，更加直观

## 2.2 多个模板参数
目前为止我们在一个函数中只使用了一个模板参数，但其实它的数量并没有限制。

但是当使用一个以上的模板参数时，函数会变得比较复杂。

![alt text](image-9.png)

这是一个拷贝函数，
注意到，为了让函数实现两个类型不同的数组的拷贝，使用了两个模板参数。

原数组加上了const修饰。第三个参数表示要拷贝的元素个数。

这里并没有使用默认参数，因为指定拷贝元素个数这个事情并没有什么默认值的概念。

然后使用了static_cast<T2\>，来使得原数组元素类型转换成目标数组的元素类型。

# c++中的类型转换
c语言的类型强制类型转换非常万能，但是也很危险。c++把c的类型转换根据使用场景拆分成了4种。
## 1. static_cast<type\>
常规类型转换，比如整形家族之间互相转换。还有不改变指针作用类型的转换，一般把某种类型的指针存放在void\*指针中之后，对这个void\*指针使用static_cast转换成原本的类型是合法的。

但是不能转换成其他类型的指针。还有就是不能转换带有const修饰的指针。
## 2. const_cast<type\>
可以对于const修饰的变量类型转换。这遗传了c转换的“去掉const特性”。但是对于常量无法使用const_cast。
## 3. reinterpret_cast<type\>
## 4. dynamic_cast<type\>

## 2.3图论与算法
给定若干点，以及若干连接这些点的连线构成的边形成的图形。这样的图可以描述很多现实问题，比如不同地点和它们之间的距离。

### 节点
图中的点称为节点。每个节点有度的概念。

### 度
与此节点存在连接的节点个数，也就是有多少条边与之相连。

### 权
边所带的值（如果这个图为有权图），比如一点到另一点的距离。

最早在欧拉研究七桥问题中就从现实问题中抽象出了由点与边构成的图。

图论的核心思想在于抽象出事物的关键要素。


图论可以帮助我们将现实问题映射到图上，再由更加抽象的图中得出算法。

比如七桥问题中欧拉就根据图给出了奇点的个数与能否不走重复的路经过所有的桥之间的算法。

这就是图论与算法的关系。

## 2.4 图论作为一种数据结构
图论中的算法要在计算机上能实现，就要求数据结构能被计算机理解，像是单纯的点线图计算机无法理解。从图中发展出了两种可以被计算机理解的数据结构，邻接矩阵，邻接表。
...
## 2.5 Dijkstra算法
在第二章节课前我就已经实现过dijkstra算法了。
要注意的是，我们一般在有权图的邻接矩阵中，用无穷大表示无法到达。在计算机中没有无穷大，所以对于无符号整形可以用-1表示最大。

但是正无穷加正数还是正无穷，但是-1加正数就变成了这个正数减1了，于是写计算路径的函数时，当此路径为-1时，不要再累加了，直接返回-1。